...
const privacyContent: Record<Locale, PrivacyContent> = {
  es: {
    ...
    paragraphs: [
      ...
      "Compartimos datos únicamente con proveedores necesarios para operar el servicio, como Resend (envío de correos), Clerk (acceso de alumnos al área privada), Neon (metadatos de materiales asignados) y Vercel (alojamiento del sitio). No vendemos ni cedemos tus datos a terceros con fines comerciales.",
      "Si accedes al área de alumno en /alumno, Clerk gestiona tu inicio de sesión. Guardamos en nuestra base de datos qué materiales (enlaces externos, por ejemplo Google Drive) están asignados a tu cuenta. No almacenamos el contenido de esos archivos en nuestros servidores.",
  ...
  en: {
    ...
    paragraphs: [
      ...
      "We share data only with service providers needed to run the site, such as Resend (email delivery), Clerk (student area sign-in), Neon (assigned materials metadata), and Vercel (hosting). We do not sell or share your data with third parties for commercial purposes.",
      "If you access the student area at /alumno, Clerk handles your sign-in. We store which materials (external links, for example Google Drive) are assigned to your account in our database. We do not store the contents of those files on our servers.",
  ...
  pl: {
    ...
    paragraphs: [
      ...
      "Udostępniamy dane wyłącznie dostawcom niezbędnym do działania serwisu, takim jak Resend (e-mail), Clerk (logowanie do strefy ucznia), Neon (metadane przypisanych materiałów) i Vercel (hosting). Nie sprzedajemy ani nie udostępniamy danych stronom trzecim w celach komercyjnych.",
      "Jeśli korzystasz ze strefy ucznia pod /alumno, logowanie obsługuje Clerk. W bazie danych zapisujemy, które materiały (linki zewnętrzne, np. Google Drive) są przypisane do Twojego konta. Nie przechowujemy treści tych plików na naszych serwerach.",
